function varargout = proses_encoding(varargin)
...
% Last Modified by GUIDE v2.5 23-Dec-2019 16:11:52
...
if ~isequal(nama_file,0)
    ...
    axes(handles.axes2)
    imshow(Img_message)
    title('Gambar yang akan disisipkan')
...
if isequal(size(Img_cover),size(Img_message))
    % membaca Hasil Region Growing Step 1 bits yang ada di edit1
    n = str2double(get(handles.edit1,'String'));
    % proses steganografi dengan metode substitusi
    ...
    axes(handles.axes3)
    imshow(Img_steg)
    title('Hasil gambar yang telah disisipkan')
...
[nama_file, nama_path] = uiputfile('*.jpg','*.tif', 'hasilpenyisipan');
% jika ada file yang dipilih maka akan mengeksekusi perintah di bawahnya
if ~isequal(nama_file,0)
